...
jQuery(document).ready(function( $ ) {
	var $posi = $('input[name="mm_menu[position]"]'),
		$brkp = $('#mm_menu_breakpoint').closest( '.section' );

	$posi.on( 'change.mm-breakpoint', function() {
		$brkp[ ( $posi.filter( ':checked' ).val() == 'left' ) ? 'show' : 'hide' ]();
	}).trigger( 'change.mm-breakpoint' );
});